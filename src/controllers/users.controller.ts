...
class UserController {
    ...
    try {
      ...
      if (error) {
        throw new AppError(
          ...
          error.message || 'Some fields are missing or incompatible type',
    ...
    try {
      ...
      if (!Number.isInteger(Number(id))) {
        throw new AppError(
          400,
          ApiErrorTypes.ValidationError,
          `Invalid ID provided; it must be a number provided id = ${id}`,
        );
      }
  ...
  async deleteUserById(
    ...
    try {
      ...
      const user = await this.userService.softDeleteUserBydId(Number(id));
